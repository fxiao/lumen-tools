...
class ModelCreator
{
     ...
     * @return string
     */
    public function create($fields, $keyName = 'id', $timestamps = true, $softDeletes = false, $relations=false)
    {
        $path = $this->getpath($this->name);

        if ($this->files->exists($path)) {
            throw new \Exception("Model [$this->name] 已存在!");
        }

        $stub = $this->files->get($this->getStub());

        $stub = $this->replaceFill($stub, $fields)
    ...
    protected function replaceRelations(&$stub, $relations)
    {
        ...
        foreach($relations as $relation) {
            $functions .= $this->dummyFunction($relation);
        }
    ...
    {
        return \$this->{$relation_name}({$class_name}::class);
    }
EOC;
    }
}
